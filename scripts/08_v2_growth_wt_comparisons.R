...
data_fname <- "~/growth_curves/WT-IF2-Growth-Curves.csv"

# Specify output location
out_fpath <- "~/growth_curves/"
...
sample_group_names <- c("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2")
...
t_gen_barplot <- ggplot(data=data_summary_stats, aes(x=group, level=sample_group_names, y=mean_t_gen, fill=group)) +
  ...
  scale_fill_manual(name = "Group", values = c("red","gray60","gray20")) +
  theme_classic() +
...
# Rate plot (r)
rate_barplot <- ggplot(data=data_summary_stats, aes(x=group, y=mean_r, fill=group)) +
  geom_bar(stat="identity", color="black") +
...
growth_curve_plot <- ggplot(data_for_scatterplt, aes(x = time, y = OD)) +
  ...
  scale_color_manual(name = "Group", values = c("red","gray60","gray20"), breaks = sample_group_names) +
...
for (i in 1:(length(sample_group_names))) {
  growth_curve_plot <- growth_curve_plot + geom_smooth(
    ...
    se = TRUE, size=2,color = (c("red","gray60","gray20"))[i])+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none", 
...
data <- as.data.frame(read.csv("~/growth_curves/WT-IF2-Growth-Curves-Summary.csv", header=T, sep=","))

#####  Data Frame #####
data$group=factor(data$group, levels=c("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2"))


type=c(rep("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2", each=5))

data$note=type
data$note=factor(data$note, levels=c("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2"))


# for barplot - create new data frame with mean and sd values - doubling time
df_mean_dt=matrix(ncol=3)
df_mean_dt=rbind(df_mean_dt, c("EcoliMG1655", mean(data$Fold.doubling[1:5]), sd(data$Fold.doubling[1:5])))
df_mean_dt=rbind(df_mean_dt, c("SL598R", mean(data$Fold.doubling[6:10]), sd(data$Fold.doubling[6:10])))
df_mean_dt=rbind(df_mean_dt, c("SL598R-pTrcHisB-WTIF2", mean(data$Fold.doubling[11:15]), sd(data$Fold.doubling[11:15])))

df_mean_dt=as.data.frame(df_mean_dt[-1,])
colnames(df_mean_dt)=c("sample","mean_dt","sd_dt")
df_mean_dt$sample=factor(df_mean_dt$sample, levels=c("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2"))


# for barplot - create new data frame with mean and sd values - doubling time
df_mean_rate=matrix(ncol=3)
df_mean_rate=rbind(df_mean_rate, c("EcoliMG1655", mean(data$Fold.growth[1:5]), sd(data$Fold.growth[1:5])))
df_mean_rate=rbind(df_mean_rate, c("SL598R", mean(data$Fold.growth[6:10]), sd(data$Fold.growth[6:10])))
df_mean_rate=rbind(df_mean_rate, c("SL598R-pTrcHisB-WTIF2", mean(data$Fold.growth[11:15]), sd(data$Fold.growth[11:15])))

df_mean_rate=as.data.frame(df_mean_rate[-1,])
colnames(df_mean_rate)=c("sample","mean_r","sd_r")
df_mean_rate$sample=factor(df_mean_rate$sample, levels=c("EcoliMG1655","SL598R","SL598R-pTrcHisB-WTIF2"))
