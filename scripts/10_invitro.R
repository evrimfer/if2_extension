...
data <- read.table(file = "~/in_vitro/IF2-WT-deltaN-PURE-Express-LUMINESCENCE-cleaned-averages-for-wt-dN-average-rep-fold.csv", header = TRUE, sep = ",", row.names = 1)
...
Neg.vs.Anc3 <- t.test(data_dataframe$`∆IF2 -Luc +NEB IF2`, data_dataframe$`∆IF2 +Luc +Anc3 IF2`)
WT.vs.Anc3 <- t.test(data_dataframe$`∆IF2 +Luc +WT IF2`, data_dataframe$`∆IF2 +Luc +Anc3 IF2`)
